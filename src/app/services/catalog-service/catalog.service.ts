...
export class CatalogService {
  ...
  getCatalog(): Observable<any> {
    return this.http.get('https://baas.kinvey.com/appdata/kid_H1tbVAPZz/landmarks', {
  ...
  addProduct(name, description,imgUrl,landmarkLocation,isVisible): Observable<any> {
    const body = JSON.stringify({name, description,imgUrl,landmarkLocation,isVisible});
    return this.http.post('https://baas.kinvey.com/appdata/kid_H1tbVAPZz/landmarks', body, {
      headers: new HttpHeaders().set('Authorization', 'Kinvey ' + localStorage.getItem('authtoken'))
        .set('Content-Type', 'application/json')
    });
  }
